        ...
        while (i < input_word.length){
            ...
            for (var j=5; j>0; j--){
                ...
                if (mappings[input_word.substring(i, i+j)]){
                    ...
                    // Arka vattu
                    if (j == 1 && ascii_arkavattu[input_word[i]]){
                        if (dependent_vowels[output[output.length-1]]){
                            var first_last = output[output.length -1];
                            var second_last = output[output.length - 2];
                            var replacement = second_last + first_last;
                            output[output.length-2] = ascii_arkavattu[input_word[i]];
                            output[output.length-1] = "್";
                            output = output.concat(replacement.split(""));
                        }
                        else{
                            var replacement = "್" + output[output.length -1];
                            output[output.length-1] = ascii_arkavattu[input_word[i]];
                            output = output.concat(replacement.split(""));
                        }
                        i++;
                    }
                    // Process for vattakshara
                    else if (j == 1 && vattaksharagalu[input_word[i]]){
                        if (dependent_vowels[output[output.length-1]]){
